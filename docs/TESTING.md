...
| `AutomaticItemIdentityServiceTest` | UNIQUE assignment, existing ID preservation, LOT creation, missing DB reconciliation, malformed identity, idempotency, ownership transfer, persistence failure isolation, owner-scoped lot reuse, different players get different lots, restart-safe lot association, lot amount update, lot amount reflects latest not accumulated, lot amount scoped to owner, lot amount zero edge case, duplicate ID no ping-pong, join reconciliation plan |
